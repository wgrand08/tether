...
def make_dlist (ft, ch, list_base, tex_base_list):
    ...
    if (ch == ord (" ")):
        ...
        glBitmap(0,0,0,0,glyph_width + 0.75,0,None)
...
class Font:
    ...
    def get(cls, facename, pixel_height):
        if cls.fonts.has_key((facename, pixel_height)):
            return cls.fonts[(facename, pixel_height)]
        return cls(facename, pixel_height)
    get = classmethod(get)
    ...
    def __init__ (self, facename, pixel_height):
        ...
        self.fonts[(facename, pixel_height)] = self
        self.textlen = None
